![Dan Ruscoe GitHub Header](https://github.com/user-attachments/assets/9cb0979c-690f-4772-a0e8-df34ffa1276f)

# Hi, I'm Dan

I'm a software developer with a background in games, web development, and API integrations.

Lately I've been working on:

* :computer: A [library for the OpenAI API](https://github.com/ruscoe/openai-php) written in PHP
* :speech_balloon: A [minimalistic web forum](https://github.com/ruscoe/tincan) written in PHP
* :skull: A [dungeon crawler game engine](https://github.com/ruscoe/dans-dungeon) written in Python
